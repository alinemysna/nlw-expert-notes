...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  const [content, setContent] = useState('')
    ...
    function handleStartRecording() {
      const isSpeeachrecognitionAPIAvailable = 'SpeechRecognition' in window
      || 'webkitSpeechRecognition' in window

      if (!isSpeeachrecognitionAPIAvailable) {
      ...
      speechRecognition.onresult = (event) => {
        const transcription = Array.from(event.results).reduce((texto, result) => {
          return texto.concat(result[0].transcript)
        }, '')
            ...
            <Dialog.Content className='fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>
              ...
              <form className='flex-1 flex flex-col'>
                ...
                {isRecording ? (
                  ...
                  className='w-full flex items-center justify-content-center gap-2 bg-slate-900 py-4 text-center text-sm text-slate-300 outline-none font-madium hover:text-slate-100'
                  >
                  ...
                  className='w-full bg-lime-400 py-4 text-center text-sm text-line-950 outline-none font-madium hover:bg-lime-500'
                  >
                    Salvar nota.
                  </button>
                )}
